...
**低信息密度内容直接 omit（密度优先于完整性）**：

- 单源、一两行、无新事件的 ticker 跟踪（如 MCHP 涨价滞后、Soitec JPM 维持评级、Taiwan SPE 数据这类）**不写入 digest**
- 标准：如果某 ticker 只能凑出 1 句话 + 没有新进展 / 没有股价异动 / 没有多源覆盖，就**完全不出现**
- digest **不追求 ticker 覆盖完整性** —— 不重要的 ticker 不写 > 写一句话凑数
- **取消 `### 简短跟踪` 段**：不再用尾部 catch-all 收纳碎片信息
...
- (b) **没有合适合并目标的，整个内容 omit**（不要为了保留而保留）
...
**不要硬塞** `####` —— 只有真正重要 / 信息量大、单独可读的 Ticker 才独立成段；只有一句话的 Ticker **直接 omit**（不写）而不是塞 bullet 凑数。
</organization>
