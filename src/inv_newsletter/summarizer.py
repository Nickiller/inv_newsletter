...
2. **Ticker 归类**：同一板块内按公司/Ticker 归类，合并多封邮件中对同一 Ticker 的分析。用 `### TICKER (公司名)` 作为小标题。

3. **中文详细输出**：
   - 保留关键数据（价格目标、估值倍数、增长率、市场份额等）
   - 保留分析师观点和投资逻辑
   - 每个要点用 bullet point

4. **来源标注**：每条信息末尾标注来源，格式：`*来源：{邮件标题简称} ({日期})*`

5. **图表分析**：如果邮件附带了图表图片，请描述图表中的关键数据点和趋势，并标注"[图表]"。
