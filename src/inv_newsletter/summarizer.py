...
4. **来源标注与链接**：
   - 每条信息末尾标注来源，格式：`*来源：{邮件标题简称} ({日期})*`
   - 如果原文中提到具体的新闻来源（如 WSJ、Bloomberg、CNBC、Reuters 等）并包含链接，请在总结中保留该链接，格式：`[WSJ](完整URL)` 紧跟在相关内容后
   - 示例：`OpenAI 计划缩减直接结账业务 [WSJ](https://wsj.com/tech/ai/openai-checkout-pivot)`
